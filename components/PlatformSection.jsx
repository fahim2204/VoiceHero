            ...
            <div className='container max-w-7xl w-full my-16'>
                ...
                <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
                    {PlatformList.map((item, index) => {
                        return (
                            <div key={index} className="rounded-3xl border flex flex-col items-center gap-5 p-5 md:p-8 lg:p-5 xl:p-8  bg-white w-full hover:bg-alpha/80 hover:text-white transition-all !duration-5000 group">
                                <Image className="size-16 object-contain" src={`/icon/${item.img}`} height={75} width={75} alt={item.name} />
                                <div className="flex flex-col items-center gap-2">
                                    <div className="font-bold text-xl md:text-2xl text-center">{item.name}</div>
                                    <div className="text-sm md:text-base text-center">{item.detail}</div>
                                </div>
                                <Link href={item.url} className="flex items-center flex-shrink-0 gap-1 text-sm md:text-base text-alpha group-hover:text-white font-bold hover:underline">View Chrome Extension<BsArrowUpRight /></Link>
                            </div>
                        );
                    })}
                </div>
